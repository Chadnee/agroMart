...
const Navbar = () => {
  ...
  return (
    ...
    <div className="flex lg:flex-row-reverse md:flex-row-reverse justify-between items-center font-serif max-w-screen backdrop-blur-sm bg-gradient-to-tr from-[#4b73b0] via-[#77bee1] to-[#26a7ed] shadow-lg fixed top-0 md:px-16 lg:px-16 py-3 left-0 w-full z-10">
      ...
      <div className="lg:flex md:flex hidden justify-end gap-16 w-1/2">
      ...
      <div className="block md:hidden lg:hidden relative w-full" ref={toggleRef}>
        <button onClick= {() => setToggle((prev) => !prev)} className="cursor-pointer">
          <FaList></FaList>
        </button>
        {
            toggle && 
                ...
                {item.subMenu ? (
                  <div>
                    <button
                      ...
                      className="text-[#1e4f68] font-semibold  flex justify-between rounded items-center cursor-pointer"
                    >
                      <span>{item.label}</span>
                    ...
                    {openMobileSubmenu && (
                      <ul className="ml-6 mt-2 flex flex-col gap-1">
                        {item.subMenu.map((sub) => (
                          <li
                            key={sub.id}
                            className="text-[#1e4f68] font-semibold flex gap-1 justify-between cursor-pointer items-center"
                          >
                            <a
                              to={sub.id}
                              smooth={true}
                  ...
                  <div>
                    <a
                      ...
                      offset={-80}
                      href={`#${item.id}`}
                      className="text-[#1e4f68] font-semibold w-full flex gap-1 cursor-pointer py-2 items-center"
                    >
                      <span>{item.label}</span>
                      <span>{item.icon}</span>
                    </a>
